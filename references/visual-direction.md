...
When one identity carrier drifts, prefer a localized edit or selective source blend. Avoid restoring the full person when that creates seams, halos, scale mismatch, inconsistent light, or a photographic sticker effect.

Use source compositing only when it improves the final artwork. Masks may be soft outside core facial features, and color, light, grain, and edge treatment may be harmonized. Never use a hard full-person cutout by default. Inspect the face at matched scale and normal viewing size; if identity is uncertain, correct the specific feature rather than demanding zero pixel difference.
...
Cross-cultural architectural logic: <off / huizhou-spatial-trial / reciprocal-modernist-trial / geometry-only-trial; confidence; source spatial principles; 20-35% target zone; formal benefit; identity safeguards; literal-motif exclusions; documented reason if off>.
