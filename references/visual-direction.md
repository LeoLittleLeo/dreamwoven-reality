...
- [People](#people)
- [Landmarks and cross-cultural architectural logic](#landmarks-and-cross-cultural-architectural-logic)
...
- [Postcard composition](#postcard-composition)
...
Treat living-subject unity as the first compositional boundary. Each visible person, animal, bird, tree, or distinct connected plant must be fully photographic or fully abstract across its complete visible form. Do not create aesthetic contrast inside a living subject. Create the contrast around it and through nonliving architecture, terrain, water, roads, shorelines, and designed structures instead.

Detect clustered life before assigning isolated-subject treatments. Treat three or more same-category subjects as a cluster when they are close enough to read together, overlap, occupy one shared ground or depth band, or repeat with a common direction and spacing. A larger dispersed set also counts when the scene clearly names it perceptually as one herd, flock, crowd, queue, procession, pedestrian stream, grove, or flower bed. Force the whole cluster into one shared abstract language. Preserve the group's extent, approximate population/density, depth bands, motion or facing direction, overlap rhythm, and a few representative silhouettes; remove faces, fleece/fur/feather detail, garment detail, leaf-by-leaf texture, and all photographic islands. For dense groups, merge members into a few unequal masses with sparse separations instead of outlining each body. If the user explicitly protects one primary person, isolate and retain that person photographically while abstracting every surrounding member.

### Mandatory primary-face lock

Before any edit, define the complete primary face as an unchanged photographic exclusion mask: forehead, facial skin, eyebrows, eyes, eyelids, nose, cheeks, lips, jaw, ears, facial hair, eyewear, hairline, expression, and their original local lighting. Preserve it exactly by default. Recognition is not enough; no generated reinterpretation, retouching, beautification, smoothing, relighting, recoloring, reshaping, texture transfer, or feature change is allowed.

Only unlock this mask when the user explicitly requests `illustrated-portrait` or anime treatment and accepts identity reinterpretation. A general request for a poster, painting, abstraction, stronger style, or paper texture does not authorize face modification.

Prefer deterministic source restoration/compositing when the edit model alters the face. Restore the full person instead of only the face when a local face patch would create seams, halos, scale mismatch, or inconsistent light. Do not repeatedly prompt a generative model to approximate an unchanged face.

Assume the edit model cannot preserve a face exactly even when prompted. After environmental generation, align the source to the output with one deterministic transform and composite the identity region from the source. Use automatic segmentation only as a starting proposal. Inspect its mask for holes, then force a separate hard lock over the complete forehead, facial skin/features, expression, eyewear, hairline, ears, jaw, and facial hair. For a fully photographic primary person, extend deterministic restoration across the complete visible body, clothing, accessories, hands, feet, and contact-bearing details when a local face patch would create inconsistent edges or light.

Keep the hard identity region binary and fully opaque. Never feather, blur, recolor, relight, tone-match, or alpha-blend inside it. Limit any seam treatment to a narrow exterior band outside the protected silhouette. At exact output scale, compute a per-channel absolute difference between the final result and aligned source under the hard lock. Require maximum pixel delta `0`; otherwise block delivery. Inspect the face at 100% and verify that the hard mask contains no internal black holes, especially across dark eyewear, hair, nostrils, mouth, facial hair, and shadowed skin that automatic segmentation may mistake for background.
...
For a photographic anchor, use the source image as the authoritative restoration reference for the full face, ears, neck, hands, fingers, hairline, and identity-critical garment/contact details. Inspect these regions at 100% zoom after every edit. Full-frame similarity is insufficient: any visible hatch, graphite stroke, canvas grain, paper fiber, paint dab, scratch, or posterized plane on exposed skin is a failure even when the face shape remains recognizable. Correct it locally from the source without regenerating successful architecture or environment.
...
## Landmarks and Cross-Cultural Architectural Logic

Use architecture and nonliving landscape anchors as the main carriers of virtual/real contrast. Within each major visible object or region, preserve one calm photographic recognition core and reconstruct one clearly contiguous zone. The balance is adaptive rather than mathematically equal, but normally neither language should fall below roughly 30% of that object's visible area. Put the division on an evidenced roofline, structural bay, joint, perspective axis, ridge, slope, shoreline, current, shadow edge, or occlusion. Avoid a generic vertical half split, random central patch, or scattered micro-fragments.

Treat buildings, mountains, cliffs, bridges, towers, monuments, pylons, masts, and horizon-breaking structures as deliberate anchors. Keep one calm photographic portion and make another portion actively incomplete through paper voids, photo slices, broken planes, displaced axes, varied line density, or creative internal structure.

### Hero-building hierarchy

When one building or landmark is visually dominant, treat it as the single architectural hero. Identify the exact silhouette, overlap, façade rhythm, opening, roofline, span, pylon, shell, tower, or other features that make it recognizable. Preserve a calm recognition-critical core and reserve the most articulate controlled deconstruction for that hero.

Treat every other nonessential building as supporting material rather than a secondary landmark. Abstract it as far as horizon, depth, scale, and directional rhythm permit. Choose one supporting language for the group:

- broad Chinese-ink washes and a few dry structural strokes;
- simplified oil-paint planes with modeled light and no façade inventory;
- sparse graphite silhouettes with very few height or roof cues;
- 2–5 geometric blocks or cut-paper masses derived from the source skyline;
- clean paper omission connected by a restrained horizontal rhythm.

Keep architectural presence binary: a source building either survives as preserved photographic evidence or is deliberately reconstructed. Do not use blur as an abstraction method for architecture. For every visible building or substantial built mass—including edge-cropped, partially occluded, distant, and low-importance structures—retain its source position, approximate footprint, directional silhouette, overlap order, and depth role. Reduce it to the chosen supporting language, but do not erase it, dissolve it into atmosphere, replace it with foliage or sky, or open empty space where the source contains a building.

Before editing, inventory architecture from left to right and assign each item one outcome: `hero preserved/reconstructed`, `supporting line system`, `supporting silhouette/ink mass`, or `supporting broad planes`. After editing, repeat the left-to-right scan against the source. If any architectural item is missing or survives only as an unstructured blur, record `L03 architecture-erasure` and reconstruct it locally without increasing its hierarchy.

Keep supporting architecture lower in contrast, detail, edge sharpness, color variety, and mark density than the hero. Remove individual windows, balconies, roof tiles, signs, and building-by-building description unless one is required for spatial explanation. Do not mix several auxiliary media across separate background buildings; one quiet system should hold them together. A supporting skyline may become almost nonliteral, but it must not create a new landmark, false location, or incorrect horizon.

Begin architectural replacement from the outer sides and move inward. Preserve the central recognition-critical core by default. Do not place a lone colored patch in the middle, outline the complete object, or let washes cross semantic boundaries.

Treat cross-cultural architectural logic as an `auto-active` architectural module: assess it on every architecture-led scene. It operates through spatial organization, massing, enclosure, circulation, structure, and solid/void rhythm; it is not a painting-medium or surface-style exchange. Do not require exact style, date, architect, or provenance. A visible formal benefit is sufficient when it improves virtual/real separation, spatial hierarchy, structural legibility, or the artistic proposition.

For broadly Western or Western-modern architecture, run a restrained `huizhou-spatial-trial` on one contiguous secondary zone covering roughly 20–35% of the visible building. Borrow Huizhou architectural thinking through layered enclosure, courtyard-like negative space, framed views, alternating solid and void, stepped wall depth, reveal/conceal sequence, compressed-to-open spatial rhythm, and asymmetric roof/wall cadence. Translate these relationships into the source building's own geometry. Preserve its recognition core, decisive silhouette, structural system, perspective, joints, load logic, light direction, and hero/support hierarchy. Expand only when the trial clearly improves the composition.

Borrow architectural principles, not cultural iconography or literal building parts. Never add horse-head walls, black tiles, vernacular whitewashed façades, physical courtyards unsupported by the source, moon gates, lattice windows, mountains, mist scenery, calligraphy, seals, dragons, decorative motifs, religious symbols, European historical ornament, or any other culturally coded content absent from the source. Never infer location, era, authorship, or provenance from the treatment. Disable experimentation only when a trial weakens recognition, structural truth, or hierarchy, or when symbolic/stereotype risk becomes material.

For broadly East Asian traditional hero architecture, a `reciprocal-modernist-trial` may reorganize one contiguous 20–35% secondary zone through Western modernist principles such as planar reduction, exposed structural grid, open/closed volume, cantilevered axis, and simplified circulation. When the architecture's broad tendency is genuinely uncertain, use a `geometry-only-trial`: choose Huizhou-derived or modernist spatial logic solely by which better clarifies the visible geometry, without naming or implying cultural identity. If the first logic provides no formal benefit, test the other once on the same or an equivalent zone. If both weaken recognition, structure, or hierarchy, record that evidence and return the zone to a neutral reconstruction language.

Creative reconstruction need not reproduce every window, arch, ridge, truss, or stratum. Preserve decisive silhouette, perspective, spatial role, structural rhythm, and one identifying feature while allowing nonliteral internal invention.

Make landmark deconstruction sparse and unmistakable. Preserve one calm, clean recognition-critical core, then select one contiguous outer or secondary zone—typically about 20–40% of the visible anchor—for large paper omissions, 2–3 broad unequal planes, one displaced axis or structural arc, and only a few confident marks with clear weight hierarchy. Let selected edges stop, disappear, and reappear. Do not cover the full landmark with uniform pencil hatching, tiny contour repetition, dirty paper noise, grunge, or an all-over sketch filter; those treatments add surface dirt without producing structural deconstruction.

## Minor Fixtures and Texture
...
Treat disposable support fixtures as complete mandatory abstraction targets. This class includes every non-hero bench, chair, stool, seat, cushion, sofa, low wall used as seating, table, and comparable furniture or support surface, whether touched or untouched. Preserve zero photographic material from the fixture. Rebuild its complete visible extent with omission, sparse perspective notation, or 1–3 broad unequal structural planes. Keep only abstract geometry required to explain support height, load-bearing edge, orientation, perspective, and contact. Composite a fully photographic person above that abstract geometry; do not preserve a photographic patch beneath a hand, foot, hip, back, elbow, or other contact point.
...
Compose the final delivery as a vertical 3:5 postcard. Use a source-derived, low-chroma ground that can shift with the scene—warm cream for ochre architecture or sunset, cool gray-blue for sea and winter, pale sage for pastoral scenes, or another restrained light tint supported by the source.

Keep a generous outer margin and one fine inner keyline. Place the unchanged original in an upper rectangle with its own fine frame, but cap it at 45% of the inner usable area and approximately 32–40% of inner height. Treat it as the clearest factual reference, not an overwhelming hero panel. Reserve at least 50% of inner usable area for the lower interpretation field. Use that lower field as an asymmetric editorial spread: selected copy levels aligned at lower left and a generously scaled generated artwork at lower right; leave the copy region quiet when copy mode is `none`. The generated interpretation should read as co-primary with the original and must not collapse into a small decorative sample.

Make the postcard copy read as a quiet caption, not a third image panel. Anchor it to the bottom-left of the lower field rather than centering it vertically. Keep the complete copy block within about 25% of canvas width and 12% of total canvas area. As a starting range, set title size to about 2.6–3.2% of canvas width, subtitle to 1.0–1.3%, and date to 0.8–1.0%. If copy exceeds the footprint, remove optional levels or shorten generated wording before increasing type. Preserve open ground above the copy and between copy and artwork.

Design the lower artwork boundary before compositing. Identify the scene's dominant directional evidence and convert it into one primary contour family:

- sea, river, harbor, waterfall: current, tide, coast, wake, river-bend, or waterline arcs;
- hill, mountain, desert, snow: ridge, slope, valley, dune, drift, or accumulation contours;
- forest, garden, foliage: canopy, branch spread, clearing, or ground-root contours;
- architecture, street, bridge: roofline, façade section, arch, span, street perspective, or structural break contours;
- person-led minimal scenes: gesture, gaze axis, garment sweep, shadow, or contact-ground contour when no stronger environmental geometry exists.

Use one dominant semantic contour and at most one quieter supporting contour. Let the boundary enter and leave the postcard ground with asymmetry and breathing room; it may be smooth, stepped, torn, dry-brushed, erased, or feathered according to the source material, but its direction must remain readable. For a coastal scene, prefer a broad river-like or tidal arc that carries the sea/horizon relationship instead of a random jagged cutout. For built scenes, prefer deliberate structural edges rather than watercolor blobs.

Do not place a visible or implied rectangle, stroke, card, shadow, glow, vignette, or uniformly blurred boundary around the generated image. Automatic edge-keying is only an implementation tool and must not choose the composition. Prefer an explicit semantic mask or an artwork generated with its intended source-grounded contour. Reject accidental white holes, corner-connected deletions, generic scallops, arbitrary organic blobs, and edge remnants that do not describe the scene.

Do not force equal halves, a square canvas, or two equally boxed images. Maintain clear hierarchy: original first, generated interpretation second, copy third. Preserve negative space between lower copy and artwork.

## Default Semantic Paper-Field Composition

Default to a rectangular paper sheet containing one large source-semantic non-rectangular content field. Follow source orientation: 3:5 portrait for portrait/square sources and 5:3 landscape for landscape sources, unless the user explicitly chooses otherwise. Derive a quiet paper ground from the source palette and let the image field occupy roughly 68–82% of the available paper field, leaving negative space on at least three sides. Do not add a keyline around the paper or content.
...
The field may touch one paper edge when direction or weight justifies it, but should not touch all four sides. Preserve source perspective and recognition-critical evidence inside it. Use a deliberately generated alpha edge or explicit semantic mask; background removal only implements the design. Reject accidental holes, corner-connected deletions, missing architecture, or any boundary that cuts through one living subject.

Place selected copy as a small annotation in paper negative space, normally lower left and outside the content field. Keep it under about 25% of paper width and 12% of paper area. Use explicit `fullbleed` mode when the user requests an edge-to-edge rectangular artwork; in that mode omit paper margins, comparison panels, and outer keylines.
...
Paper field: <default processed rectangular sheet; orientation; source-adjacent ground; 68-82% content occupancy>.
...
Living-subject integrity: <inventory every visible person, animal, bird, tree, or connected plant; assign each fully photographic or fully abstract; keep transitions outside every living silhouette>.
Living clusters: <detect close/overlapping/repetitive same-category groups first; force flock/herd/crowd/queue/procession/pedestrian flow/grove/flower bed into one shared fully abstract language; record any explicitly protected primary exception>.
Main person: <none / fully photographic / fully abstract faceless / illustrated portrait>; preserve fully photographically by default.
Identity lock: <deterministic source alignment/composite; hard face/full-person regions; no internal feathering; mask-hole inspection; max pixel delta 0>.
Nonliving duality: <for each major architecture/terrain/water/road region, identify photographic core, contiguous abstract zone, and source-grounded transition; exclude disposable support fixtures>.
Tall landmark: <preserved core>; <deconstructed portion and medium>; keep treatment inside its boundary.
Supporting architecture: <left-to-right inventory; source location and selected line/silhouette/plane treatment for every visible building; no blur or erasure>.
Disposable support fixtures: <inventory every bench/chair/stool/seat/cushion/sofa/low seating wall/table; require complete abstraction and zero photographic texture; preserve support/contact only through abstract geometry and person-over-fixture compositing>.
...
Cross-cultural architectural logic: <off / huizhou-spatial-trial / reciprocal-modernist-trial / geometry-only-trial; confidence; source spatial principles; 20-35% target zone; formal benefit; identity locks; literal-motif exclusions; documented reason if off>.
...
- **`P00 face-lock-breach`:** discard generative approximations and restore the exact source face or complete source person with deterministic compositing. Repair every internal mask hole; force binary source opacity across face, eyewear, hairline, ears, jaw, and other locked pixels; move all feathering outside the identity silhouette. Compare final vs aligned source under the hard mask and block delivery until `max_pixel_delta=0`. This remains blocking except for explicitly authorized `illustrated-portrait`/anime treatment.
- **`P02 texture-contamination`:** restore the complete fully photographic person from the source, including face, skin, hair, garments, accessories, and hands; remove every environment-derived mark crossing the body.
...
- **`B02 living-subject-split`:** trace the complete affected person or organism and choose one language for all of it. Restore a primary person fully from the source by default; otherwise rebuild an authorized secondary subject fully and remove every photographic island.
...
- **`V01 weak-virtual-real-contrast`:** strengthen one or more source-semantic abstract zones while retaining calm photographic evidence; reject both filter-like similarity and wholesale illustration that loses the productive contrast.
- **`V02 nonliving-duality-failure`:** restore a photographic recognition core or add a contiguous abstract reconstruction so each major architecture and nonliving landscape carrier visibly contains both languages, normally at no less than roughly 30% each.
...
- **`D02 disposable-support-retention` (blocking):** identify the complete connected bench, chair, stool, seat, cushion, sofa, low seating wall, table, or comparable support. Remove every photographic fixture pixel, including beneath the body, and rebuild the entire object with omission, sparse perspective lines, or 1–3 broad unequal planes. Restore the photographic person above it and preserve contact through abstract load-bearing geometry. Recheck the full fixture for upholstery, masonry, wood grain, seams, repeated units, wear, and source microdetail; any retained example still blocks delivery unless the user explicitly protected the fixture as a subject.
...
- **`M01 unjustified-module`:** remove an unrequested or conceptually weak cloud, cinematic title, or illustrated portrait treatment. Do not remove the cross-cultural architectural-logic assessment; it has its own mandatory `M05` gate.
...
- **`M03 cross-cultural-architecture-overreach`:** remove painting-style substitution and literal cultural motifs. Reduce the intervention to one contiguous 20–35% secondary architectural zone; preserve silhouette, structural identity, perspective, joints, load logic, light, recognition core, and hierarchy. For a Huizhou spatial trial, retain only layered enclosure, courtyard-like void, framed-view, solid/void, stepped-depth, reveal/conceal, compressed/open, and roof/wall-rhythm principles expressed through the source building's own geometry. Remove horse-head walls, black tiles, vernacular white façades, invented courtyards, moon gates, lattice windows, calligraphy, seals, historical cues, and provenance implications.
- **`M05 cross-cultural-architecture-omission`:** run and record the architectural-logic assessment before generating every architecture-led scene. Apply a Huizhou spatial trial to Western/Western-modern hero architecture, a reciprocal modernist trial when useful for East Asian traditional hero architecture, or a geometry-only trial when classification is uncertain. If the trial is absent, add it locally; if it failed, document the specific recognition, structure, or hierarchy damage that justifies `off`. Neutral watercolor, ink, oil, graphite, or paper treatment alone never satisfies this gate.
...
- **`O01 wrong-mode`:** rebuild the requested postcard or standalone structure and remove elements belonging to the other mode.
...
- **`O04 postcard-proportion-failure`:** reduce the upper original to no more than 45% of inner usable area and roughly 32–40% of inner height; expand the lower artwork/copy field to at least 50% and ensure the generated artwork reads as co-primary rather than thumbnail-sized.
- **`O05 postcard-copy-dominance`:** shrink the copy block, bottom-anchor it at lower left, keep it within roughly 25% of canvas width and 12% of total canvas area, and omit optional text before allowing it to compete with either image field.
- **`O06 semantic-content-field-failure`:** rebuild default processed output on a rectangular paper sheet; replace full bleed, a rectangular/oval/rounded/blob/vignette field, or an automatic cutout with one dominant source-semantic contour, preserve 68–82% active content and paper on at least three sides, and restore evidence lost to accidental holes.
